...
void buttonPress(){
    ...
    if(SSL_write(sslClient, buf, strlen(buf)) == -1){
...
//    }
    ...
    if(SSL_set_fd(sslClient, sock) == 0){
        fprintf(stderr, "Error setting ssl fd\n");
        exit(1);
    }
    ...
    //send ID
    if(SSL_write(sslClient, id, 13) == -1){
    ...
    if(r==-1){
        ...
        if(fds[0].revents & POLLIN){
            int readstatus = SSL_read(sslClient, inputbuff+validPtr, 256-validPtr);
            if(readstatus < 0){
        ...
        commandPtr+=1;
         ...
         if(run_flag){
            ...
            //printf("%s\n", msgbuf);
            if(SSL_write(sslClient, msgbuf, strlen(msgbuf)) == -1){
